...
def debug_personal_schedules():
    """개인 일정 디버깅용 API - 모든 일정 조회 (보안 강화)"""
    try:
        # 클라이언트가 보낸 employee_id를 신뢰하지 않음
        # 서버에서 인증된 사용자 정보만 사용
        ...
        from models.schemas import PersonalScheduleListResponse, PersonalScheduleResponse
        
        schedules = PersonalSchedule.query.filter_by(employee_id=employee_id).all()
        debug_data = []
        
        for schedule in schedules:
            schedule_data = PersonalScheduleResponse(
                id=schedule.id,
                title=schedule.title,
                ...
                employee_id=schedule.employee_id,
                created_at=schedule.created_at if hasattr(schedule, 'created_at') else datetime.now(),
                updated_at=schedule.updated_at if hasattr(schedule, 'updated_at') else None
            )
            debug_data.append(schedule_data.model_dump())
        
        # 응답 데이터 검증
        response_data = PersonalScheduleListResponse(
