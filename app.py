...
# --- 지능형 약속 잡기 관련 모델 ---
class IntelligentSchedule(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    host_id = db.Column(db.String(50), nullable=False)
    title = db.Column(db.String(100), nullable=False)
    description = db.Column(db.Text, nullable=True)
    selected_participants = db.Column(db.Text, nullable=False)  # JSON 형태로 저장
    suggested_dates = db.Column(db.Text, nullable=False)  # JSON 형태로 저장
    status = db.Column(db.String(20), default='voting')  # 'voting', 'confirmed', 'cancelled'
    confirmed_date = db.Column(db.String(20), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    def __init__(self, host_id, title, description, selected_participants, suggested_dates):
        self.host_id = host_id
        self.title = title
        self.description = description
...
class ScheduleVote(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    schedule_id = db.Column(db.Integer, db.ForeignKey('intelligent_schedule.id'), nullable=False)
    voter_id = db.Column(db.String(50), nullable=False)
    voted_date = db.Column(db.String(20), nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    def __init__(self, schedule_id, voter_id, voted_date):
        self.schedule_id = schedule_id
        self.voter_id = voter_id
        self.voted_date = voted_date

@app.route('/intelligent/schedule', methods=['POST'])
def create_intelligent_schedule():
    ...
    try:
        ...
        title = data.get('title')
        description = data.get('description', '')
        selected_participants = json.dumps(data.get('selected_participants', []))
        suggested_dates = json.dumps(data.get('suggested_dates', []))
        
        if not host_id or not title or not data.get('selected_participants'):
...
@app.route('/intelligent/schedule/<int:schedule_id>/vote', methods=['POST'])
def submit_schedule_vote(schedule_id):
    """약속 날짜 투표"""
    try:
        data = request.get_json()
        ...
        if not voter_id or not voted_date:
            return jsonify({'error': '투표 정보가 누락되었습니다.'}), 400
        
        # 기존 투표가 있는지 확인
        existing_vote = ScheduleVote.query.filter_by(
        ...
        if existing_vote:
            # 기존 투표 수정
            existing_vote.voted_date = voted_date
        else:
            # 새 투표 생성
            vote = ScheduleVote(
                schedule_id=schedule_id,
...
@app.route('/intelligent/schedule/<int:schedule_id>/confirm', methods=['POST'])
def confirm_schedule(schedule_id):
    ...
    try:
        ...
        schedule = IntelligentSchedule.query.get(schedule_id)
        if not schedule:
            return jsonify({'error': '약속을 찾을 수 없습니다.'}), 404
        
        if schedule.host_id != host_id:
            return jsonify({'error': '약속을 확정할 권한이 없습니다.'}), 403
        
        # 투표 결과 집계
        votes = ScheduleVote.query.filter_by(schedule_id=schedule_id).all()
        vote_counts = {}
        ...
        if not vote_counts:
            return jsonify({'error': '투표가 없습니다.'}), 400
        
        # 가장 많이 투표된 날짜 선택
        confirmed_date = max(vote_counts.items(), key=lambda x: x[1])[0]
...
@app.route('/intelligent/schedule/<int:schedule_id>', methods=['GET'])
def get_schedule_details(schedule_id):
    ...
    try:
        ...
        for vote in votes:
            vote_counts[vote.voted_date] = vote_counts.get(vote.voted_date, 0) + 1
            if vote.voted_date not in voter_details:
                voter_details[vote.voted_date] = []
            voter_details[vote.voted_date].append(vote.voter_id)
        
        # 참가자 정보 조회
        participants = json.loads(schedule.selected_participants)
        participant_details = []
        for participant_id in participants:
            user = User.query.filter_by(employee_id=participant_id).first()
            if user:
        ...
        return jsonify({
            'schedule': {
                ...
                'suggested_dates': json.loads(schedule.suggested_dates)
            },
            'votes': vote_counts,
...
@app.route('/intelligent/schedule/my/<employee_id>', methods=['GET'])
def get_my_schedules(employee_id):
    """내가 참여한 지능형 약속 목록"""
    try:
        # 내가 호스트인 약속들
        hosted_schedules = IntelligentSchedule.query.filter_by(host_id=employee_id).all()
        
        # 내가 참가자인 약속들
        all_schedules = IntelligentSchedule.query.all()
        participated_schedules = []
        
        for schedule in all_schedules:
            participants = json.loads(schedule.selected_participants)
            if employee_id in participants and schedule.host_id != employee_id:
                participated_schedules.append(schedule)
        ...
        return jsonify({
            'hosted': [format_schedule(s) for s in hosted_schedules],
            'participated': [format_schedule(s) for s in participated_schedules]
        }), 200
        
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/intelligent/schedule/common-dates', methods=['POST'])
def get_common_available_dates():
    """선택된 참가자들의 공통 가능 날짜 조회"""
    try:
        data = request.get_json()
        participant_ids = data.get('participant_ids', [])
        
        if not participant_ids:
            return jsonify({'error': '참가자 정보가 필요합니다.'}), 400
        ...
        while current_date <= end_date:
            ...
            # 모든 참가자가 해당 날짜에 일정이 없는지 확인
            all_available = True
            
            for participant_id in participant_ids:
                # 개인 일정 확인
                personal_schedule = PersonalSchedule.query.filter_by(
                    employee_id=participant_id,
                    schedule_date=date_str
                ).first()
