...
from backend.presentation.api.error_mapping import ErrorResponse, ErrorCode, HTTPStatusMapping


class BaseAPI:
    """기본 API 클래스 - HTTP 규약 준수"""
    
    def success_response(self, data: Any, status_code: int = 200, message: Optional[str] = None) -> Response:
        """성공 응답 - 적절한 상태코드 사용"""
        response_data = {
            "data": data,
    ...
    def rate_limit_response(self, retry_after: Optional[int] = None) -> Response:
    ...
    def internal_error_response(self, error_id: Optional[str] = None) -> Response:
    ...
    def service_unavailable_response(self, retry_after: Optional[int] = None) -> Response:
